👀 Um Pato e um Sonho
   
   ✅ Este é um novo jogo de sobrevivência que eu estou criando sozinho, direto pelo celular. Não é um jogo pronto e está MUITO longe de ser 100% completo, mas a ideia é evoluir ele junto com quem quiser acompanhar.
   
   🛠️ Como eu faço (0% mentiras):
No Celular, eu Programo tudo pelo Acode com a ajuda do Gemini (IA), que me ajuda a resolver os bugs e criar as lógicas.
   
   👍🏻 Sem enrolação: Eu subo o código aqui e o jogo atualiza sozinho para vocês graças ao Web2App. Você não precisa baixar de novo toda vez que eu mudar algo!
   
 👀 Onde ver o progresso:

   - Eu posto tudo o que estou fazendo no YouTube. Se o código quebrar ou se eu conseguir uma função nova, você vai ver lá:
(https://www.youtube.com/@UmPatoeUmSonho)

 ⚠️ Papo reto:
   - O jogo vai ter bug, vai travar e muita coisa ainda não funciona. Eu crio as versões, elas atualizam automaticamente no seu celular/navegador e a gente vai arrumando juntos.
   
   🦆 Com um pato e um sonho, a gente vai montando esse universo!