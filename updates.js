...
function drawTile(ctx, tile) {
    ...
    switch(tile.type) {
        case 'grass':
            ...
            for(let i = 0; i < 5; i++) {
                const gx = tile.x + Math.random() * TILE_SIZE;
                const gy = tile.y + Math.random() * TILE_SIZE;
        ...
        case 'dirt':
            ...
            for(let i = 0; i < 3; i++) {
                const dx = tile.x + Math.random() * TILE_SIZE;
                const dy = tile.y + Math.random() * TILE_SIZE;
        ...
        case 'stone':
            ...
            for(let i = 0; i < 4; i++) {
                const sx = tile.x + Math.random() * TILE_SIZE;
                const sy = tile.y + Math.random() * TILE_SIZE;
...
function render() {
    ...
    if(hour >= 5 && hour < 7) {
        ...
        const t = (hour - 17) / 2;
        skyColor = lerpColor('#87ceeb', '#ff6b6b', t);
    } else {
        skyColor = '#0a0f1a';
    }
    ...
    const TILE_SIZE = 60;
    const CHUNK_SIZE = 16;
    const startChunkX = Math.floor((game.cam.x - 100) / (CHUNK_SIZE * TILE_SIZE));
    const endChunkX = Math.ceil((game.cam.x + cw + 100) / (CHUNK_SIZE * TILE_SIZE));
    const startChunkY = Math.floor((game.cam.y - 100) / (CHUNK_SIZE * TILE_SIZE));
    const endChunkY = Math.ceil((game.cam.y + ch + 100) / (CHUNK_SIZE * TILE_SIZE));
        ...
        for(let cy = startChunkY; cy <= endChunkY; cy++) {
            ...
            chunk.forEach(tile => {
                if(tile.type === 'grass' || tile.type === 'dirt' || tile.type === 'stone') {
                    if(tile.x >= game.cam.x - TILE_SIZE && tile.x <= game.cam.x + cw + TILE_SIZE &&
                       tile.y >= game.cam.y - TILE_SIZE && tile.y <= game.cam.y + ch + TILE_SIZE) {
                        drawTile(ctx, tile);
                    }
                }
            });
...
console.log('✅ Updates.js v2.2.0 carregado!');